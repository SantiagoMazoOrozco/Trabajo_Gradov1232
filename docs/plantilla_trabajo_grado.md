...
Esta sección debe incluir, dentro del mismo documento, el título, el resumen y las palabras clave en español e inglés (Resolución 023 de 2015, Art. 02).
...
Nota: La tabla de contenido puede generarse automáticamente en el procesador de texto. En Markdown, muchas herramientas la generan con `[[TOC]]` o plugins equivalentes.
...
> Nota: si es requerido, incluya lista de ilustraciones/gráficas. Idealmente, generada de forma automática por el editor.
...
> Nota: similar a la lista de figuras. Generar automáticamente si es posible.
...
> Incluya solo los símbolos/abreviaturas usados. Orden alfabético.
...
> Sugerencias de formato (NTC 1486 / APA): márgenes 2.54 cm (ext.) y 3.6 cm (interna), fuente recomendada (p. ej., Arial 11 para párrafos), numeración romana en preliminares y arábiga desde introducción.
...
- Modelar la arquitectura de un SMA que orqueste ingesta, preprocesamiento y preparación de datos para entrenamiento.
- Implementar el prototipo SMA integrando al menos RF y SVM en la fase de entrenamiento, utilizando JADE.
- Evaluar eficiencia computacional y sostenibilidad energética frente a una arquitectura centralizada, midiendo tiempo, throughput, CPU, memoria y energía (J/MB).
- Analizar e interpretar resultados cuantitativos para validar eficiencia y sostenibilidad, identificando implicaciones técnicas, éticas y operativas.
...
> Las ilustraciones deben mencionarse en el texto y ubicarse en la misma página o la siguiente. Numeración consecutiva. En tablas, títulos en la parte superior, sin abreviaturas innecesarias.
...
Consulte `docs/refs.bib` (BibLaTeX, estilo IEEE) para las entradas bibliográficas estructuradas. En LaTeX se imprime con `\printbibliography`; en Markdown, cite de forma consistente y mantenga autores/año completos.

<!--
Instrucciones de uso:
- RELLENAR los campos en el front matter (encima) y los corchetes [RELLENAR] en el cuerpo.
- Si compila a PDF con herramientas como pandoc, los marcadores <!-- pagebreak --> pueden usarse como saltos de página.
- Ajuste tipografías y normas específicas según guía de la Facultad.
-->
